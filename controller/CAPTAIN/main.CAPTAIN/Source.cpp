...
#define DISTANCE_BETWEEN_XTE_POINTS 1
#define WAYPOINT_CIRCLE_ARRIVAL 5
#define TOOLWIDTH 20
...
std::tuple<double, double> calculateRhumbDistanceAndBearing(const Coordinate coordinate_one,
	const Coordinate coordinate_two)
{
...
// 1) Using GeographicLib
// 2) Some help taken from https://www.movable-type.co.uk/scripts/latlong.html
double calculateXte(const Coordinate coordinate_one, const Coordinate coordinate_two, const Pose curr_pose)
{
	...
	const double ds0 = DISTANCE_BETWEEN_XTE_POINTS;
...
(const Coordinate current_position, const std::vector<Coordinate> key_method_positions, const int method)
{
	...
	case 1: //Coverage Rectangle
	{
		...
		GeographicLib::Rhumb rhumb_one(GeographicLib::Constants::WGS84_a(), GeographicLib::Constants::WGS84_f());
		GeographicLib::Rhumb rhumb_two(GeographicLib::Constants::WGS84_a(), GeographicLib::Constants::WGS84_f());
		GeographicLib::Rhumb rhumb_three(GeographicLib::Constants::WGS84_a(), GeographicLib::Constants::WGS84_f());
		...
		const double ds0 = TOOLWIDTH; // Nominal distance between points on help lines
...
std::vector<Coordinate> calculatePointsFromSegments(std::vector<Coordinate> path_segments, const int method)
{
	...
	case 1: //Coverage Rectangle
	{
		for (std::vector<Coordinate>::iterator it = path_segments.begin(); it != prev(path_segments.end()); ++it)
		{
			...
			else //This is a vertical line; we need to wrap around the outside of the rectangle to adjust our position
			{
				...
				// Nominal distance between points
				const double ds_circle = DISTANCE_BETWEEN_POINTS;
...
std::tuple<double, std::string, double, int, double, double> calculateTelegramData(
	Pose curr_pose, std::vector<Coordinate> path_, std::vector<Coordinate> completed_path_)
{
	...
	if (std::get<0>(next_path_curr_location_distance) < WAYPOINT_CIRCLE_ARRIVAL)
	{
	...
	if (completed_path_.size() < 1)
	{
		...
		xte = calculateXte(path_.at(0), path_.at(1), curr_pose);
	}
	//Normal operation
	else
	{
		...
		xte = calculateXte(path_.at(completed_path_.size() - 1), path_.at(completed_path_.size()), curr_pose);
	}
...
std::string appendChecksumToTelegram(std::string telegram)
{
...
int main()
{
	Coordinate start = Coordinate{ 56.0, 10.0 };
	Pose pose = { Coordinate{ 56.0, 10.0 }, 30 };
	double GPSSpeed = 10;

	//Point to point
	//std::vector<Coordinate> coords = {{56.01, 10.01}};

	//Rectangle
	std::vector<Coordinate> coords = { { 56.001, 10.001 },{ 56.002, 10.002 } };

	//Construct the path_ vector as a list of the calculated segments
	std::vector<Coordinate> path_segments = calculatePathSegments(start, coords, 1);

	//for (int i = 0; i < path_segments.size(); ++i)
	//{
	//std::cout << std::fixed << std::setprecision(8) << path_segments[i].Latitude_ << " " << path_segments[i].Longitude_ << std::endl;
	//}
	//std::cout << std::endl;
	...
	std::tuple<double, std::string, double, int, double, double> data =
		calculateTelegramData(pose, path_, completed_path_);
	...
	//Construct the telegram without the checksum, true relative_direction is left, false is right
	const std::string vtg_telegram_no_chksum = "GPVTG,,,,," + std::to_string(GPSSpeed / 1.852) + ",N," +
		std::to_string(GPSSpeed) + ",K";
	...
	//Print
	std::cout << apb_telegram_with_checksum << std::endl << std::endl;
	std::cout << vtg_telegram_with_checksum << std::endl << std::endl;
}
